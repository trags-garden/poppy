"""Tests for CaptureReconciler (automatic candidate reconciliation).
...
def test_ambiguous_candidate_retrieves_once(engine: SeedEngine, monkeypatch: pytest.MonkeyPatch) -> None:
    """Double-retrieval kill: the ambiguous band reuses the neighbours
    the prefilter already fetched — one engine.retrieve per candidate, not two."""
    engine.ingest(_mk("existing", "Use all-MiniLM for embeddings."))
    ...
    monkeypatch.setattr(engine, "retrieve", counting_retrieve)
    # A low-confidence verdict proves we reached the ambiguous band (the LLM ran)
    ...
    assert decision.action is Action.ADD  # ambiguous → LLM → low confidence → ADD
    assert calls["n"] == 1, "the ambiguous band must not trigger a second engine.retrieve"
