...
def _codex_transcript(tmp_path: Path, n: int = 6) -> Path:
    rows = [{"type": "session_meta", "payload": {"type": "session_meta", "id": "codex-session"}}]
    for i in range(n):
        role = "user" if i % 2 == 0 else "assistant"
        ...
        rows.append(
            {
                ...
                "payload": {
                    ...
                    "content": [
                        {
                            "type": content_type,
                            "text": f"Codex turn {i}: a substantive deployment architecture and schema decision {i}",
                        }
                    ],
                },
            }
...
def test_codex_worker_disables_soft_cap_and_stamps_codex_provenance(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, enabled: SeedEngine
) -> None:
    monkeypatch.setattr(
        "poppy.consolidation.call_llm",
        lambda *a, **k: [{"type": "decision", "content": "Keep Codex cadence capture active past the soft cap."}],
    )
    path = _codex_transcript(tmp_path)
    for _ in range(DEFAULT_SOFT_CAP_K):
        record_capture(tmp_path, "codex-session")

    n = consolidate_capture_event({"session_id": "codex-session", "transcript_path": str(path), "cwd": str(tmp_path)})

    assert n == 1
    assert capture_count(tmp_path, "codex-session") == DEFAULT_SOFT_CAP_K
    assert enabled.list_all(limit=10)[0].source.type == "codex"
...
def test_codex_user_prompt_cadence_ignores_soft_cap(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from click.testing import CliRunner
    ...
    path = _codex_transcript(tmp_path)
    for _ in range(DEFAULT_SOFT_CAP_K):
        record_capture(tmp_path, "codex-hook")
    payload = json.dumps(
        {
            "cwd": str(tmp_path),
            "session_id": "codex-hook",
            "transcript_path": str(path),
            "prompt": "a long enough Codex prompt to pass the recall gate",
        }
    )
    ...
    capture_spawns = [spawn for spawn in spawned if spawn and list(spawn[1:]) == ["hook", "_capture-worker"]]
    assert len(capture_spawns) == 1
