...
def test_migrate_skips_a_row_changed_during_embed(tmp_path: Path, monkeypatch) -> None:
    """If content changes while _embed runs, the vector is not written."""
    db = tmp_path / "memories.db"
    _seed_with_baseline_and_attach_embeddings(
        db, [(_make_memory("m0", "kept"), "fake-bi-encoder-v1", np.array([1.0, 2.0, 3.0, 4.0], dtype=np.float32))]
    )
    SeedEngine(db_path=db).ingest(_make_memory("m1", "OLD content"))

    class _MutatingEngine(_FakeEngine):
        def _embed(self, text: str) -> np.ndarray:
            # Simulate a concurrent seed edit landing while this embed runs.
            conn = connect(db)
            conn.execute("UPDATE memories SET content = ? WHERE id = ?", ("NEW content", "m1"))
            conn.commit()
            conn.close()
            return super()._embed(text)

    done = migrate(_MutatingEngine(), db, MigrateFilters())
    # m1 changed under the embed, so no vector was written for it.
    assert done == 0
    got = connect(db).execute("SELECT COUNT(*) FROM memory_embeddings WHERE id = ?", ("m1",)).fetchone()[0]
    assert got == 0


def test_migrate_embeds_current_content_not_a_stale_snapshot(tmp_path: Path, monkeypatch) -> None:
    """A write between list_targets and the embed must not tag a stale vector as current."""
    db = tmp_path / "memories.db"
    seen: list[str] = []
    ...
    # A bloom-shaped store (has memory_embeddings) with an already-compatible row,
    # plus a seed-written row that has NO embedding — the anti-join migrate target.
    _seed_with_baseline_and_attach_embeddings(
        db, [(_make_memory("m0", "kept"), "fake-bi-encoder-v1", np.array([1.0, 2.0, 3.0, 4.0], dtype=np.float32))]
    )
    SeedEngine(db_path=db).ingest(_make_memory("m1", "OLD content"))

    real_list_targets = list_targets

    def _stale_then_mutate(db_path, model_id, filters):
        targets = real_list_targets(db_path, model_id, filters)
        conn = connect(db)
        conn.execute("UPDATE memories SET content = ? WHERE id = ?", ("NEW content", "m1"))
        conn.commit()
        conn.close()
        return targets  # still carries "OLD content"

    monkeypatch.setattr("poppy.engine.migration.list_targets", _stale_then_mutate)
    migrate(_RecordingEngine(), db, MigrateFilters())
    assert seen == ["NEW content"], seen
