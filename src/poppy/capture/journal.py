...
class CaptureRecord:
    ...
    # Capture host / source app ("claude-code", "codex", ...). Optional so legacy
    # journal lines written before this field parse back as source=None.
...
def record(
    ...
    *,
    ...
    ``source`` is the capture host / source app so host-specific liveness checks
    (e.g. the doctor's Codex line) can filter to their own records."""
    rec = CaptureRecord(
...
def read_last(poppy_dir: Path, *, source: str | None = None) -> CaptureRecord | None:
    """The most recent capture record, or None if nothing captured yet.

    When ``source`` is given, only records from that capture host are considered
    (legacy lines with no source field never match a host filter), so a
    host-specific liveness check does not read another host's capture.
    """
