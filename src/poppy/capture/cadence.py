...
def register_turn(poppy_dir: Path, session_id: str) -> int:
    ...
    Runs in a fresh process on every UserPromptSubmit, so the increment goes
    through the locked read-modify-write (``_state.update``) to avoid lost turns.
    """
    ...
    return _state.update(poppy_dir, _mutate)
