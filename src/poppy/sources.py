...
# Host coding-agent CLI executable (as returned by `consolidation.detect_host_cli`)
# -> canonical source app. Auto-capture classifies the host CLI to pick an
# extraction backend; that same classification IS the memory's provenance, so it
# must resolve here rather than being hardcoded at each capture site. The Claude
# Code CLI's executable is ``claude`` but its source app is spelled ``claude-code``.
_HOST_CLI_TO_SOURCE: dict[str, str] = {
    "claude": "claude-code",
    "cursor-agent": "cursor",
    "codex": "codex",
    "gemini": "gemini",
...
_CAPTURE_HOST_TO_SOURCE: dict[str, str] = {
...
# Source app stamped on an auto-captured memory when the host CLI cannot be
# classified (no transcript path, or nothing recognized on PATH — e.g. an
# OpenAI-compat fallback with no host CLI). Preserves the historical stamp so
# pre-fix captures stay attributed to Claude Code rather than going unattributed.
DEFAULT_CAPTURE_SOURCE = "claude-code"

# Source apps used by bulk-import integrations. These may carry synthetic
...
def source_from_host_cli(cli: str | None) -> str:
    """Resolve the source app for an auto-captured memory from its host CLI.

    ``cli`` is the executable name `consolidation.detect_host_cli` picked for the
    session (``claude`` / ``codex`` / ``gemini``), or ``None`` when no host CLI
    was detected. Known CLIs map through ``_HOST_CLI_TO_SOURCE``; an unknown but
    non-empty CLI is slugified so its real provenance is preserved (mirroring
    `normalize_client`); missing/unclassifiable falls back to
    ``DEFAULT_CAPTURE_SOURCE`` so a capture is never left unattributed.
    """
    if not cli:
        return DEFAULT_CAPTURE_SOURCE
    key = cli.strip().lower()
    return _HOST_CLI_TO_SOURCE.get(key) or _slugify(key) or DEFAULT_CAPTURE_SOURCE


def source_from_capture_host(host: str | None, *, fallback_cli: str | None = None) -> str:
    """Resolve auto-capture provenance from the transcript adapter's host."""
    if host:
        key = host.strip().lower()
        return _CAPTURE_HOST_TO_SOURCE.get(key) or _slugify(key) or DEFAULT_CAPTURE_SOURCE
    return source_from_host_cli(fallback_cli)
