...
def get_goose_config_dir() -> Path:
    ...
    Honors Poppy's explicit override first, then Goose's ``GOOSE_PATH_ROOT``,
    then ``XDG_CONFIG_HOME``. Defaults to ``~/.config/goose`` on macOS/Linux.
    ...
    if goose_path_root:
        return Path(goose_path_root) / "config"
    if os.name == "nt":
