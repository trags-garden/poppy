...
5. Decrypt it with the private key (which never leaves this process) and store
   it via the standard config-save path, which puts it in the OS keychain when a
   backend is available and otherwise falls back to config.json.
...
def run_device_code_flow(api_url_override: str | None = None) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if poll.status_code == 200:
            ...
            save_config(cfg)
            from poppy.config import trags_api_key_location

            key_location = trags_api_key_location(poppy_dir)
            if key_location == "keychain":
                click.echo("\n✓ Connected. Your API key is stored in your OS keychain.")
            elif key_location == "config-file":
                # The resolved path, not a literal ~/.poppy: POPPY_DIR moves it.
                click.echo(f"\n✓ Connected. Your API key is saved in {poppy_dir / 'config.json'} (0600).")
            else:
                # keychain-unreadable or none: this session cannot see where the
                # key landed, so name no location rather than assert the file.
                click.echo(
                    "\n✓ Connected. This session could not confirm where the key was stored; "
                    "run `poppy doctor` to check."
                )
