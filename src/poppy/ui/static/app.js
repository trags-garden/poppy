  ...
  if (state.selected) {
    ...
    if (fresh) {
      ...
      try {
        ...
        state.selected = null;
        // Under 880px the detail covers the list, so a selection that
        // evaporates would strand the reader on an empty pane.
        showList();
        renderDetail();
      }
    }
  ...
  for (const m of state.items) {
    ...
    li.innerHTML = `
      ...
      <div class="body">
        ...
        <div class="meta">
          <span class="type">${escapeHtml(m.memory_type)}</span>
          ${tombTag}
          ${proj}
          ${src}
          ${relBadge}
          ...
          return `
            <div class="feed-item" data-id="${escapeHtml(m.id)}">
              ...
              <div class="body">
                ...
                <div class="meta">
                  <span>${escapeHtml(m.memory_type)}</span>
                  <span>${proj}</span>
...
function renderDetail() {
  ...
  if (!m) {
    pane.innerHTML = `
      <button class="btn ghost detail-back" id="d-back">← All memories</button>
...
// The facet rail is a sheet below 880px; this handle opens it, and choosing
// anything inside it closes it again.
...
// Anything that changes which memories are listed has to surrender both
// overlays, or the narrow layout keeps showing the memory that was open when
// the query changed.
function showList() {
  closeFilters();
...
function wireFilters() {
  // Delegated, so the control works in every detail render and in the markup
  // index.html ships before the first one.
  document.getElementById("detail").addEventListener("click", (e) => {
    if (e.target.closest(".detail-back")) showList();
  });
