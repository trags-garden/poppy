...
- `poppy setup` now refuses unparseable client configs before changing files or
  starting the daemon, preserving Claude Code settings instead of silently
  replacing user permissions, environment, model and hooks with Poppy's hooks.
  Cursor's existing hooks backup and replacement behavior is unchanged.
